...
class ADSBSignalDataset(Dataset):
    ...
    def _npy_label_candidates(self, x_path: str) -> List[str]:
        dir_name, base = os.path.split(x_path)
        root, ext = os.path.splitext(base)
        candidates: List[str] = []

        def add(root_name: str) -> None:
            if not root_name:
                return
            cand = os.path.join(dir_name, root_name + ext)
            if cand not in candidates:
                candidates.append(cand)

        add(root + "_y")
        if root.startswith("X_"):
            add("Y_" + root[2:])
            add("y_" + root[2:])
        if root.startswith("x_"):
            add("y_" + root[2:])
            add("Y_" + root[2:])
        if root.startswith("X") and not root.startswith("X_"):
            add("Y" + root[1:])
            add("y" + root[1:])
        if root.startswith("x") and not root.startswith("x_"):
            add("y" + root[1:])
    ...
    def _resolve_npy_label_path(self, x_path: str) -> Tuple[Optional[str], List[str]]:
        candidates = self._npy_label_candidates(x_path)
    ...
    def __init__(
        ...
        closed_set_num: Optional[int] = None,
        ...
        if ext == ".npy":
            ...
            if label_path is not None:
                y_path = os.path.abspath(label_path)
                if not os.path.exists(y_path):
                    raise FileNotFoundError(f"labels file not found: {y_path}")
            else:
                y_path, candidates = self._resolve_npy_label_path(self.path)
                if y_path is None:
                    tried = ", ".join(candidates)
                    raise FileNotFoundError(f"labels file not found for {self.path}. Tried: {tried}")
        ...
        if closed_set_num is not None:
            limit = int(closed_set_num)
            if limit <= 0:
                raise ValueError("closed_set_num must be positive")
            if labels.size:
                keep = labels[self.indices] < limit
...
class LoaderConfig:
    ...
    path_train_label: Optional[str] = None
    path_val_label: Optional[str] = None
    path_test_label: Optional[str] = None
    closed_set_num: Optional[int] = None
...
def build_dataloaders(cfg: LoaderConfig) -> Dict[str, Any]:
    ...
    ds_tr = ADSBSignalDataset(
        path=cfg.path_train,
        label_path=cfg.path_train_label,
        closed_set_num=cfg.closed_set_num,
    ...
    def _mk_eval_loader(
        ...
        label_opt: Optional[str],
    ) -> Tuple[Optional[ADSBSignalDataset], Optional[DataLoader]]:
        if path_opt is None:
            return None, None
    ...
    ds_va, val_loader = _mk_eval_loader(cfg.path_val, cfg.path_val_label)
    ds_te, test_loader = _mk_eval_loader(cfg.path_test, cfg.path_test_label)
