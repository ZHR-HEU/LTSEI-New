...
class LDAMLoss(WeightedBaseLoss):
    ...
    IMPORTANT: 
    - If using a standard Linear classifier (not normalized), set scale=1.0
    - If using a Cosine/Normalized classifier, use scale=30.0 (or similar)
    
    Args:
        class_counts: Number of samples per class
        max_margin: Maximum margin value (C in paper), default 0.5
        scale: Logit scaling factor. Use 1.0 for Linear classifier, 30.0 for Cosine classifier
        drw_start_epoch: Epoch to start Deferred Re-Weighting (0 = no delay)
        reweight_power: Power for inverse frequency weights in DRW (default 0.25 as in paper)
        use_normalized_margin: If True, normalize margins to [0, max_margin]; 
                               If False, use C/n^(1/4) directly (paper default)
    """
    ...
    def __init__(
        ...
        scale: float = 1.0,  # Changed default to 1.0 for Linear classifier
        drw_start_epoch: int = 0,
        reweight_power: float = 0.25,  # Paper uses 1/4 power
        use_normalized_margin: bool = False,  # False = paper's original formula
        **kwargs
    ):
        ...
        # Print warning if scale seems wrong
        if scale > 5.0:
            warnings.warn(
                f"LDAMLoss: scale={scale} is large. "
                f"For standard Linear classifier, use scale=1.0. "
                f"scale>1 is only appropriate for Cosine/Normalized classifiers."
            )
    ...
    def _compute_margins(self, device) -> torch.Tensor:
        ...
        Paper formula: Δ_j = C / n_j^(1/4)
        where C is max_margin (typically 0.5)
        """
        if self._margins is None:
            ...
            if self.use_normalized_margin:
                # Normalized version: scale to [0, max_margin]
                # Largest class gets smallest margin, smallest class gets max_margin
                raw = 1.0 / torch.pow(counts, 0.25)  # 1/n_c^(1/4)
                # Normalize: min margin for largest class, max margin for smallest
                raw_min = raw.min()
                raw_max = raw.max()
                if raw_max - raw_min > 1e-8:
                    margins = (raw - raw_min) / (raw_max - raw_min) * self.max_margin
                else:
                    margins = torch.full_like(raw, self.max_margin / 2)
            else:
                # Original paper formula: Δ_j = C / n_j^(1/4)
                # C is max_margin, directly used as the constant
                margins = self.max_margin / torch.pow(counts, 0.25)
            
            self._margins = margins
            
            # Debug info
            print(f"[LDAMLoss] Margin range: [{margins.min().item():.4f}, {margins.max().item():.4f}]")
            print(f"[LDAMLoss] Scale: {self.scale}, DRW start epoch: {self.drw_start_epoch}")
            
        return self._margins.to(device)

    def _compute_drw_weights(self, device) -> torch.Tensor:
        """Compute DRW (Deferred Re-Weighting) weights."""
    ...
    def forward(self, logits: torch.Tensor, targets: torch.Tensor, **kwargs) -> torch.Tensor:
        ...
        # For standard Linear classifier (scale=1): just subtract margin
        # For Cosine classifier (scale=s): logits are already s*cos(θ), subtract margin
        adjusted_logits = logits.clone()
        batch_indices = torch.arange(logits.size(0), device=device)
        adjusted_logits[batch_indices, targets] -= margin_values

        # Scale: only apply if using normalized classifier (scale > 1)
        # For Linear classifier, scale should be 1.0
        if self.scale != 1.0:
            adjusted_logits = adjusted_logits * self.scale
...
class LOSLoss(BaseLoss):
    """
    Label Over-Smoothing (LOS) loss from ICLR 2025.

    LOS uses a large label_smoothing value so the target distribution is close
    to uniform while keeping the true class slightly above 1/K.

    You can specify ONE of:
    - smoothing: PyTorch label_smoothing (epsilon), default 0.98
    - true_prob: desired probability for the true class (p_true)
    - delta: offset above uniform (p_true = 1/K + delta)
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.delta = delta
        if self.true_prob is not None and self.delta is not None:
            raise ValueError("LOSLoss: set only one of true_prob or delta")

    def _validate_smoothing(self, smoothing: float) -> float:
        if not 0.0 <= smoothing <= 1.0:
            raise ValueError(f"LOSLoss: smoothing must be in [0, 1], got {smoothing}")
        return float(smoothing)

    def _resolve_smoothing(self, num_classes: int) -> float:
        if num_classes <= 1:
            return 0.0

        if self.true_prob is not None:
            p_true = float(self.true_prob)
        elif self.delta is not None:
            p_true = (1.0 / num_classes) + float(self.delta)
        else:
            return self._validate_smoothing(self.smoothing)

        uniform = 1.0 / num_classes
        if not uniform < p_true < 1.0:
            raise ValueError(
                f"LOSLoss: true_prob must be in ({uniform:.6f}, 1.0), got {p_true}"
            )

        smoothing = (1.0 - p_true) / (1.0 - uniform)
        return self._validate_smoothing(smoothing)

    def forward(self, logits: torch.Tensor, targets: torch.Tensor, **kwargs) -> torch.Tensor:
        targets = TensorUtils.to_1d_long(targets)
        smoothing = self._resolve_smoothing(logits.size(-1))
        return F.cross_entropy(
            logits, targets,
