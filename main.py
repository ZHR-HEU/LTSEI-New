...
# 训练工具（整合自 common, optim_utils, train_eval, stage2, trainer_logging, training_utils）
from training import (
    # 设备与种子
    setup_seed, parse_gpu_ids, setup_device,
    # 优化器与调度器
    build_scheduler_for_stage, build_optimizer, build_criterion, build_optimizer_with_groups,
    # 训练与评估
    ...
    # 早停与检查点
    create_early_stopping, ModelCheckpointer, TrainingManager, MetricsTracker,
    # 日志
    TrainingLogger,
)
...
def load_checkpoint(path: str, map_location):
    try:
        ...
        try:
            ...
            print(f"[WARN] weights_only load failed ({exc}); falling back to weights_only=False")
...
def main(cfg: DictConfig):
    ...
    stage1_cfg = getattr(cfg, 'stage1', None)
    stage2_cfg = getattr(cfg, 'stage2', None)
    stage1_load_only = bool(stage1_cfg and getattr(stage1_cfg, 'load_weights_only', False))
    stage1_load_only_run_stage2 = bool(stage1_cfg and getattr(stage1_cfg, 'load_weights_only_run_stage2', False))
    stage1_ckpt_hint = getattr(stage1_cfg, 'checkpoint', None) if stage1_cfg else None
    will_train_stage1 = not stage1_load_only
    will_run_stage2 = bool(stage2_cfg and stage2_cfg.enabled and (not stage1_load_only or stage1_load_only_run_stage2))

    print("\n配置概览:")
    print(f"  模型: {cfg.model.name}")
    print(f"  数据: batch={cfg.data.batch_size} | 训练集={cfg.data.path_train} | 测试集={cfg.data.path_test}")
    print(f"  Stage-1: {'训练' if will_train_stage1 else '仅加载'} | 损失={cfg.loss.name} | 采样={cfg.sampling.name} | lr={cfg.training.lr}")
    if stage1_load_only:
        print(f"    权重: {stage1_ckpt_hint}")
        print(f"    继续 Stage-2: {'是' if stage1_load_only_run_stage2 else '否'}")
    print(f"  Stage-2: {'启用' if cfg.stage2.enabled else '禁用'} | 实际执行: {'是' if will_run_stage2 else '否'}")
    if cfg.stage2.enabled:
        print(f"    模式={cfg.stage2.mode} | epochs={cfg.stage2.epochs} | lr={cfg.stage2.lr} | 优化器={cfg.stage2.optimizer}")
        print(f"    采样={cfg.stage2.sampler} | 冻结BN={'是' if cfg.stage2.freeze_bn else '否'} | warmup_epochs={getattr(cfg.stage2, 'warmup_epochs', 0)}")
    print(f"  长尾不平衡: {'是' if getattr(cfg, 'create_imbalance', False) else '否'} | ratio={cfg.data.imbalance_ratio}")
    ...
    loader_cfg = LoaderConfig(
        ...
        path_train_label=getattr(cfg.data, "path_train_label", None),
        path_val_label=getattr(cfg.data, "path_val_label", None),
        path_test_label=getattr(cfg.data, "path_test_label", None),
        closed_set_num=getattr(cfg.data, "closed_set_num", None),
    ...
    # 处理验证集划分 - 必须在人工不平衡之前，确保验证集保持均衡分布
    if cfg.data.path_val is None and loaders['val'] is None:
        print(f"未提供验证集，从原始均衡训练集划分 {cfg.data.val_ratio * 100:.0f}%...")
        ...
        train_indices, val_indices = train_test_split(
            original_indices, test_size=cfg.data.val_ratio,
            stratify=original_labels, random_state=cfg.seed
        )

        # 验证集：保持均衡分布，使用无增强的独立数据集（避免数据泄露）
        val_dataset = ADSBSignalDataset(
            path=cfg.data.path_train,
            label_path=getattr(cfg.data, "path_train_label", None),
            closed_set_num=getattr(cfg.data, "closed_set_num", None),
            indices=val_indices,
            target_length=cfg.data.target_length,
        ...
        print(f"  验证集样本数: {len(val_indices)} (保持均衡分布)")

        # 创建人工不平衡（如需要）- 仅对训练索引应用
        if cfg.create_imbalance:
            print(f"\n创建人工不平衡 (ratio: {cfg.data.imbalance_ratio})")
            # 只从训练索引中选择，避免数据泄露到验证集
            train_labels_subset = original_labels[train_indices]

            # 在训练子集内创建长尾分布
            longtail_local_indices = make_long_tailed_indices(
                train_labels_subset, num_classes,
                cfg.data.imbalance_ratio, cfg.seed
            )

            # 将局部索引映射回原始数据集的全局索引
            longtail_global_indices = train_indices[longtail_local_indices]
            ...
            imbalanced_dataset = ADSBSignalDataset(
                ...
                label_path=getattr(cfg.data, "path_train_label", None),
                closed_set_num=getattr(cfg.data, "closed_set_num", None),
                indices=longtail_global_indices,
                target_length=cfg.data.target_length,
                normalize=getattr(cfg.data, 'normalize', True),
                seed=cfg.seed
            )
            ...
            print(f"  训练集样本数: {len(longtail_global_indices)} (人工不平衡)")
        else:
            # 无人工不平衡，使用划分后的训练子集
            train_subset = Subset(train_dataset, train_indices)
            train_subset.labels = original_labels[train_indices]
            train_loader = DataLoader(
    ...
    stage1_load_only = bool(stage1_cfg and getattr(stage1_cfg, 'load_weights_only', False))
    stage1_load_only_run_stage2 = bool(stage1_cfg and getattr(stage1_cfg, 'load_weights_only_run_stage2', False))
    skip_stage1_training = stage1_load_only
    skip_stage2 = stage1_load_only and (not stage1_load_only_run_stage2)
    ...
    val_time_stage1 = 0.0
    stage1_train_time = 0.0
    stage1_total_time = 0.0

    if skip_stage1_training:
        stage1_ckpt = getattr(stage1_cfg, 'checkpoint', None) if stage1_cfg else None
        if not stage1_ckpt or not os.path.exists(stage1_ckpt):
            raise FileNotFoundError(f"[Stage-1] checkpoint not found: {stage1_ckpt}")
        checkpoint = load_checkpoint(stage1_ckpt, map_location=device)
        model.load_state_dict(checkpoint['model_state_dict'])
        if isinstance(checkpoint, dict) and isinstance(checkpoint.get('metrics'), dict):
            best_val_acc_stage1 = float(checkpoint['metrics'].get('acc', 0.0))
        print(f"\n=== Stage-1 跳过训练，仅加载权重测试: {stage1_ckpt} ===")
        if cfg.stage2.enabled:
            if skip_stage2:
                print("[Stage-1] load_weights_only 已开启，Stage-2 将被跳过")
            else:
                print("[Stage-1] load_weights_only 已开启，Stage-2 将继续训练")

        print("\n[Stage-1] 测试(加载权重)")
        stage1_test_metrics, stage1_test_analysis, _, _, stage1_test_timing = evaluate_with_analysis(
            model, test_loader, criterion, device, analyzer, class_counts,
            cfg.evaluation.eval_logit_adjust, cfg.evaluation.eval_logit_tau
        )
        print(f"  准确率: {stage1_test_metrics['acc']:.2f}% | 平衡准确率: {stage1_test_metrics['balanced_acc']:.2f}%")
        print(f"  测试时间: {stage1_test_timing['milliseconds']:.2f} ms")
        if stage1_test_analysis.get('group_wise'):
            print("  分组性能:")
            for group_name, metrics in stage1_test_analysis['group_wise'].items():
                if metrics:
                    print(f"    {group_name.capitalize():8s}: "
                          f"Acc={metrics['accuracy']:.2f}% | F1={metrics['f1']:.2f}% | Support={metrics['support']}")
    else:
        # 8. Stage-1 训练
        ...
        for epoch in range(1, stage1_epochs + 1):
            ...
            if not math.isfinite(train_metrics['loss']):
                print(f"\n[Stage-1] 检测到非有限损失，停止训练 (epoch {epoch})")
            ...
            if not math.isfinite(val_metrics['loss']):
                print(f"\n[Stage-1] 验证损失为非有限值，停止训练 (epoch {epoch})")
                actual_stage1_epochs = epoch
                break
    ...
    if (not skip_stage2) and cfg.stage2.enabled and cfg.stage2.mode != 'tau_norm_only':
        ...
        if (not hasattr(cfg.stage2, 'warmup_epochs') or cfg.stage2.warmup_epochs is None or
                not hasattr(cfg.stage2, 'warmup_multiplier') or cfg.stage2.warmup_multiplier is None):
            with OmegaConf.open_dict(cfg):
                if not hasattr(cfg.stage2, 'warmup_epochs') or cfg.stage2.warmup_epochs is None:
                    cfg.stage2.warmup_epochs = 0
                if not hasattr(cfg.stage2, 'warmup_multiplier') or cfg.stage2.warmup_multiplier is None:
                    cfg.stage2.warmup_multiplier = getattr(cfg.scheduler, 'warmup_multiplier', 1.0)
        ...
        scaler2 = GradScaler('cuda', enabled=(cfg.amp and device.type == 'cuda'))

        stage2_epoch_offset = stage1_epochs if not skip_stage1_training else 0

        for epoch in range(1, stage2_epochs + 1):
            if stage2_sampler is not None and hasattr(stage2_sampler, 'set_epoch'):
                stage2_sampler.set_epoch(epoch)
            ...
            logger.start_epoch(stage2_epoch_offset + epoch, lr=current_lr)

            train_metrics = train_one_epoch(
                model, stage2_loader, criterion2, optimizer2, device,
                logger, stage2_epoch_offset + epoch, cfg.training.grad_clip, cfg.amp, scaler2
            )

            if not math.isfinite(train_metrics['loss']):
                print(f"\n[Stage-2] 检测到非有限损失，停止训练 (epoch {epoch})")
            ...
            if not math.isfinite(val_metrics['loss']):
                print(f"\n[Stage-2] 验证损失为非有限值，停止训练 (epoch {epoch})")
                actual_stage2_epochs = epoch
                break
            ...
            logger.log_epoch_end(stage2_epoch_offset + epoch, train_metrics, val_metrics,
        ...
        if os.path.exists(best_path2):
            checkpoint = load_checkpoint(best_path2, map_location=device)
            model.load_state_dict(checkpoint['model_state_dict'])
